...
### ======================================================================== ###
#
PlotModelFeatImp <- function(w.avg.imps, d=NULL, type, title='placeholder') {
...
PlotOneModelMetrics <- function(metrics, 
                                model.type, 
                                d.m = c("Balanced Accuracy"), # Desired Metrics to be shown,
                                title = "placeholder"
                                ) {
...
### ======================================================================== ###
#
InitialiseImportanceDF <- function(cols, byClass=F) {
...
### ======================================================================== ###
#
PlotTune.mtry.ntree.RandomForest <- function(df,
...
### ======================================================================== ###
#
RepCV.RandomForest <- function(df, 
                               ntree=500,
                               mtry=4,
                               nrepeat=1, 
                               nfolds=5,
...
### ======================================================================== ###
#
PlotTune.CP.MinSplit.DecisionTree <- function(df,
                                              cps = seq(0, 0.2, 0.02),
...
### ======================================================================== ###
#
PlotTune.CP.DecisionTree <- function(df,
                         cps = seq(0, 0.2, 0.02),
                         split.crit = "gini",
                         nrepeat=50,
...
### ======================================================================== ###
#
RepCV.DecisionTree <- function(df, 
                               cp = 0.01,
                               minsplit = 20,
                               split.crit = "gini",
                               nrepeat=1, 
...
### ======================================================================== ###
#
RepCV.MultinomialLogisticRegression <- function(df,
                                                reg.alpha=NULL,
                                                nrepeat=1,
                                                nfolds=5,
                                                seed=NULL,
...
### ======================================================================== ###
#
FindClosestValue <- function(point, # list(long=x, lat=y)
                             DF)
  {
  rows.x <- which(round(DF$x,4) == round(point$long[1],4))
  subset.DF <- DF[rows.x, ]
  r <- which.min(abs(point$lat - DF$y))
  return(DF[r,])
}
...
### ======================================================================== ###
#
ExtractRasterValuesToMatrix <- function(raster, 
                                        summary = FALSE, 
                                        xbounds=NULL, 
                                        ybounds=NULL) 
  {
  vals <- getValues(raster)
  which.na <- which(is.na(vals))
  if (summary) {print(summary(vals[-which.na]))}
  if (is.null(xbounds)||is.null(ybounds)) {stop("X and Y bounds must be entered")}
  df <- data.frame(t(matrix(vals, ncol=nrow(raster))))[xbounds, ybounds]
  return(df)
}

### ======================================================================== ###
#
ConfirmDimAllData <- function(listOfDFs, 
                              intendedDim) 
  {
  for (df in listOfDFs) {
    if (dim(df)[1] != intendedDim[1] || dim(df)[2] != intendedDim[2]) {
      print("Not all datasets have the same dimensions")
      return(F)
    }
  }
  return(T)
...
### ======================================================================== ###
#
RowToPoint <- function(r) {
  return(list(long=r$long, lat=r$lat))
}


### ======================================================================== ###
#
XYToPoint <- function(x, 
                      y) 
  {
  return(list(long=x, lat=y))
}